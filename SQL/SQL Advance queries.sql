...
SELECT * FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
...
SELECT MODEL_NAME, [STATE],DIM_DATE.DATE
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_LOCATION ON FACT_TRANSACTIONS.IDLOCATION=DIM_LOCATION.IDLOCATION
					   INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
					   WHERE [YEAR]>=2005 GROUP BY [STATE],DIM_DATE.DATE, MODEL_NAME

--Q2.WHAT STATE IN US BUYING MORE 'SAMSUNG' PHONES?

SELECT [STATE],MANUFACTURER_NAME
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
...
COUNT(DIM_CUSTOMER.IDCUSTOMER) [NO. OF TRANSACTION]
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_Customer ON FACT_TRANSACTIONS.IDCustomer=DIM_Customer.IDCustomer
                       INNER JOIN DIM_LOCATION ON FACT_TRANSACTIONS.IDLOCATION=DIM_LOCATION.IDLOCATION
					   GROUP BY MODEL_NAME,ZIPCODE,[STATE] ORDER BY [NO. OF TRANSACTION] DESC

--Q4. SHOW THE CHEAPEST CELLPHONE.

SELECT TOP 1 MODEL_NAME,
MIN(TOTALPRICE) [PRICE]
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
GROUP BY MODEL_NAME ORDER BY [PRICE] ASC

--Q5. FIND THE AVERAGE PRICE FOR MODEL IN TOP 5 MANUFACTURERS IN TERM OF SALES QUANTITY AND ORDER BY AVERAGE PRICE

SELECT MANUFACTURER_NAME, MODEL_NAME, QUANTITY, AVG(TOTALPRICE) [AVERAGE] 
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_MANUFACTURER ON DIM_MODEL.IDManufacturer=DIM_MANUFACTURER.IDManufacturer
WHERE EXISTS(SELECT TOP 5 MANUFACTURER_NAME,
SUM(QUANTITY) [TOTAL QUANTITY]
FROM FACT_TRANSACTIONS  INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                        INNER JOIN DIM_MANUFACTURER ON DIM_MODEL.IDManufacturer=DIM_MANUFACTURER.IDManufacturer
GROUP BY MANUFACTURER_NAME ORDER BY [TOTAL QUANTITY] DESC)
GROUP BY MANUFACTURER_NAME, MODEL_NAME,QUANTITY ORDER BY [AVERAGE] DESC


--Q6. LIST THE NAMES AND THE AVERAGE SPENDING IN 2009, WHERE THE AVERAGE IS HIGHER THAN 500.
...
GROUP BY [YEAR], CUSTOMER_NAME 
						HAVING [YEAR]=2009 AND AVG(TOTALPRICE)>(SELECT
AVG(TOTALPRICE)
...
--Q7. LIST IF THERE IS ANY MODEL THAT WAS IN TOP 5 IN TERMS OF QUANTITY SIMULTANEOUSLY IN 2008, 2009, 2010.

SELECT * FROM (SELECT TOP 5 MODEL_NAME,QUANTITY,
COUNT(QUANTITY) [NO. OF PHONES]
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
WHERE [YEAR] IN (2008)
GROUP BY MODEL_NAME,QUANTITY ORDER BY Quantity DESC)ABCD
INTERSECT
SELECT * FROM (SELECT TOP 5 MODEL_NAME, QUANTITY,
COUNT(QUANTITY) [NO. OF PHONES]
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
...
GROUP BY MODEL_NAME,QUANTITY ORDER BY QUANTITY DESC) XYZ
INTERSECT
SELECT * FROM (SELECT TOP 5 MODEL_NAME,QUANTITY,
COUNT(QUANTITY) [NO. OF PHONES]
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
WHERE [YEAR] IN (2010)
GROUP BY MODEL_NAME,QUANTITY ORDER BY QUANTITY DESC) UVW


--Q8. SHOW THE MANUFACUTRERS THAT SOLD THE PHONE IN 2010 BUT NOT IN 2009.
...
--Q9. SHOW THE MANUFACTURER WITH 2ND TOP SALES IN THE YEAR 2009 AND THE MANUFACTURER WITH 2ND TOP SALE IN THE YEAR 2010.


SELECT TOP 1 * FROM (SELECT TOP 2 MANUFACTURER_NAME,
sum(TOTALPRICE) [SALES]
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_MANUFACTURER ON DIM_MODEL.IDMANUFACTURER=DIM_MANUFACTURER.IDMANUFACTURER
					   INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
					   GROUP BY MANUFACTURER_NAME,[YEAR] HAVING [YEAR] IN (2009) ORDER BY [SALES] DESC) ABCD ORDER BY [SALES] ASC
...
FROM FACT_TRANSACTIONS INNER JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDMODEL=DIM_MODEL.IDMODEL
                       INNER JOIN DIM_MANUFACTURER ON DIM_MODEL.IDMANUFACTURER=DIM_MANUFACTURER.IDMANUFACTURER
					   INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
					   GROUP BY MANUFACTURER_NAME,[YEAR] HAVING [YEAR] IN (2010) ORDER BY [SALES] DESC) ABCD ORDER BY [SALES] ASC


--Q10. FIND 100 CUSTOMERS AND THEIR AVERAGE SPENDING, AVERAGE QUANTITY BY EACH YEAR. ALSO FIND THEIR PERCENTAGE CHANGE IN THEIR SPENDING.

--A

SELECT IDCUSTOMER,(CASE WHEN [YEAR] =2002 THEN TOTALPRICE END) AS SPEND_2002,
 (CASE WHEN [YEAR] =2003 THEN TOTALPRICE END) AS SPEND_2003,
 (CASE WHEN [YEAR] =2004 THEN TOTALPRICE END) AS SPEND_2004,
 (CASE WHEN [YEAR] =2005 THEN TOTALPRICE END) AS SPEND_2005,
 (CASE WHEN [YEAR] =2006 THEN TOTALPRICE END) AS SPEND_2006,
 (CASE WHEN [YEAR] =2007 THEN TOTALPRICE END) AS SPEND_2007,
 (CASE WHEN [YEAR] =2008 THEN TOTALPRICE END) AS SPEND_2008,
 (CASE WHEN [YEAR] =2009 THEN TOTALPRICE END) AS SPEND_2009,
 (CASE WHEN [YEAR] =2010 THEN TOTALPRICE END) AS SPEND_2010
 FROM (SELECT TOP 100 DIM_Customer.IDCustomer, [YEAR],TOTALPRICE,
 AVG (TOTALPRICE) [AVERAGE SPEND],
 AVG(QUANTITY) [AVERAGE QUANTITY]
 FROM FACT_TRANSACTIONS INNER JOIN DIM_Customer ON FACT_TRANSACTIONS.IDCustomer=DIM_Customer.IDCustomer
                        INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
 GROUP BY DIM_Customer.IDCustomer, [YEAR],TOTALPRICE ORDER BY [AVERAGE SPEND] DESC)ABCD

--B


SELECT (SPEND_2003-SPEND_2002)/SPEND_2002*100 [P%_2003],
(SPEND_2004-SPEND_2003)*100/SPEND_2003 [P%_2004],
(SPEND_2005-SPEND_2004)*100/SPEND_2004 [P%_2005],
(SPEND_2006-SPEND_2005)*100/SPEND_2005 [P%_2006],
(SPEND_2007-SPEND_2006)*100/SPEND_2006 [P%_2007],
(SPEND_2008-SPEND_2007)*100/SPEND_2007 [P%_2008],
(SPEND_2009-SPEND_2008)*100/SPEND_2008 [P%_2009],
(SPEND_2010-SPEND_2009)*100/SPEND_2009 [P%_2010]
FROM
(SELECT IDCUSTOMER,(CASE WHEN [YEAR] =2002 THEN TOTALPRICE END) AS SPEND_2002,
 (CASE WHEN [YEAR] =2003 THEN TOTALPRICE END) AS SPEND_2003,
 (CASE WHEN [YEAR] =2004 THEN TOTALPRICE END) AS SPEND_2004,
 ...
 FROM FACT_TRANSACTIONS INNER JOIN DIM_Customer ON FACT_TRANSACTIONS.IDCustomer=DIM_Customer.IDCustomer
                        INNER JOIN DIM_DATE ON FACT_TRANSACTIONS.DATE=DIM_DATE.DATE
 GROUP BY DIM_Customer.IDCustomer, [YEAR],TOTALPRICE ORDER BY [AVERAGE SPEND] DESC)ABCD)T1
